...
--
-- Host: 127.0.0.1
-- Generation Time: May 20, 2026 at 07:10 AM
...
CREATE TABLE `orders` (
  ...
  `order_status` enum('pending','confirmed','shipped','delivered','cancelled','awaiting_payment','awaiting_cod') DEFAULT 'pending',
...
CREATE TABLE `users` (
  ...
  `status` tinyint(1) DEFAULT 1 COMMENT '0->icatuve\r\n1->active and defult also no need to take inout while signup\r\n\r\n',
...
--
ALTER TABLE `order_payment`
  ADD CONSTRAINT `foreign key` FOREIGN KEY (`order_id`) REFERENCES `orders` (`id`);
COMMIT;
...
ALTER TABLE `vendors` ADD `password` VARCHAR(255) NOT NULL AFTER `hash`;

ALTER TABLE `products` ADD `vendor_id` INT(20) NULL DEFAULT NULL AFTER `id`;
